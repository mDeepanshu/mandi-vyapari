...
export class TransactionScreenComponent implements OnInit {
  ...
  ngOnInit() {
      this.sharedService.getMyLedger("194ab93ce0a", '2025-08-08', '2025-08-11').subscribe((data:any) => {
        ...
        for (let i = transactionsData.length - 1; i >= 0; i--) {
          ...
          if (curr_date === element.date) {
            ...
            groupedData[groupedData.length - 1].items.push({ itemName: element.itemName, dr: element.dr, cr: element.cr });
          } else {
            ...
            groupedData.push({
              ...
              dr: element.dr,
              cr: element.cr,
              items: [{ itemName: element.itemName, dr: element.dr, cr: element.cr }]
            });
          }
        }
